...
ax.plot(np.array(model_runs),np.array(measured_phis),'ko')
ax.set_xlabel('Model runs')
ax.set_ylabel('log10(phi)')
#x_ticks = np.arange(1,36,5)
#x_ticks_labels = np.arange(0,35,5)
...
ax.text(0,600,'target measurement objective function',fontsize=8)

fig.tight_layout()
fig.savefig(os.path.join(figure_dir,'Figure8.pdf'),dpi='figure',format='pdf')
plt.close()